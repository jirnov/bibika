...
ServiceRecord* ServiceRecordBuilder::fromJSON(const QString& jsonString, QObject* parent)
{
  ...
  setField(
    "eventType", [](const auto& v) { return str2EventType(v.toString()); }, &ServiceRecord::setEventType);

  setField(
    "name", [](const auto& v) { return v.toString(); }, &ServiceRecord::setName);

  setField(
    "notes", [](const auto& v) { return v.toString(); }, &ServiceRecord::setNotes);

  setField(
    "price", [](const auto& v) { return v.toInt(); }, &ServiceRecord::setPrice);

  setField(
    "mileage", [](const auto& v) { return v.toInt(); }, &ServiceRecord::setMileage);

  setField(
    "serviceDate",
    [](const auto& v) { return QDate::fromString(v.toString(), Qt::ISODate); },
    &ServiceRecord::setServiceDate);

  setField(
    "repeatAfterDistance", [](const auto& v) { return v.toInt(); }, &ServiceRecord::setRepeatAfterDistance);

  setField(
    "repeatAfterMonths", [](const auto& v) { return v.toInt(); }, &ServiceRecord::setRepeatAfterMonths);

  setField(
    "hasRepeatAfterDistance", [](const auto& v) { return v.toBool(); }, &ServiceRecord::setHasRepeatAfterDistance);

  setField(
    "hasRepeatAfterMonths", [](const auto& v) { return v.toBool(); }, &ServiceRecord::setHasRepeatAfterMonths);
